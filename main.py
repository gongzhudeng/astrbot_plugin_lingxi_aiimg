...
"""
    ...
    async def aiimg_generate(
        ...
        模式说明：
        - 编辑模式（edit）：禁止自行使用。只有用户明确要求修改已有图片时才能使用。用户必须发送或引用一张图片，并提出具体修改要求。没有用户明确要求或没有可编辑图片时，不得使用编辑模式。
        - 自拍模式（selfie_ref）：默认模式。自拍模式不局限于露脸自拍，也不要求画面必须出现人物的脸或完整身体。以第一人称视角拍摄正在看到或拿着的画面也属于自拍模式，可以拍摄眼前的食物、物品、环境或其他日常所见。应将其理解为人物本人正在拍摄的照片，而不是普通文生图。
        - 文生图模式（text）：不允许自行使用，只有用户明确要求使用文生图时才能使用。

        省略 mode 或传入 auto 时必须使用自拍模式。不要因为画面中没有人物或没有露脸而改用文生图模式。自拍参考图不存在时必须报告失败，不得降级为文生图。
        提示词不能写得过度暴露或违规，否则会审核失败；请使用合规、克制的描述。

        Args:
            prompt(string): 完整的生成或修改要求。默认按自拍模式理解；自拍模式可以描述第一人称正在拍摄的画面，不强制露脸。
            mode(string): 默认 selfie_ref（自拍模式）。edit 是仅限用户明确要求修改已有图片时使用的编辑模式；text 仅限用户明确要求文生图时使用。auto 仅作为兼容别名，等同于 selfie_ref。
