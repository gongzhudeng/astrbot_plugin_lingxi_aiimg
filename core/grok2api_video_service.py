...
class Grok2ApiVideoService:
    def __init__(self, *, settings: dict):
        ...
        self.api_keys = [
            k for k in (api_keys if isinstance(api_keys, list) else [api_keys]) if k
        ]
            ...
            async with httpx.AsyncClient(
                ...
                if image_bytes:
                    ...
                    resp = await client.post(
                        self.api_url, headers=headers, data=data_fields, files=files
                    )
                else:
                    ...
                    payload["n"] = 1
                    resp = await client.post(
                        self.api_url, headers=headers_json, json=payload
                    )
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                if video_url:
                    ...
                    logger.info(
                        f"[Grok2ApiVideo] 成功: 耗时={t_end - t_start:.2f}s, url={video_url[:80]}..."
                    )
                    return video_url
